...
$(document).ready(function() {
    ...
    (function loadRSS() {
      ...
      var yql = "https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20rss%20where%20url%3D'http%3A%2F%2Fblog.ffii.org/feed/'&format=json&callback=";
      $.getJSON(yql, function(data) {
        ...
        for(index in posts) {
          ...
          posts[index].description = posts[index].description.replace(/(src)="(?!http|\/\/)/gi, 'src="http://blog.ffii.org');
          //do the same thing with links (i.e. for tags)
          posts[index].description = posts[index].description.replace(/(href)="(?!http|\/\/)/gi, 'href="http://blog.ffii.org');
          ...
          $('#result').append("<div class='post'><h3 class='title'>"+posts[index].title+"</h3> \
                              ...
                              <a href='"+posts[index].link+"'><div class='link'>See more at FFII</div></a> \
                              </div>");
          if(index == 0) {
